...
// AI 社区：帖子、Skill 与 AI 应用均以云端数据为准，本地只保存用户主动下载的副本。
momoRouter.get("/community/posts",requireMember,asyncHandler(async(_req,res)=>{res.json(await momoRepo.communityPosts(String(res.locals.memberId)));}));
momoRouter.post("/community/posts",requireMember,asyncHandler(async(req,res)=>{const memberId=String(res.locals.memberId),member=await memberRepo.getById(memberId);if(!member){res.status(404).json({error:"账号不存在"});return;}const title=String(req.body?.title||"").trim(),body=String(req.body?.body||"").trim();if(!title||!body){res.status(400).json({error:"标题和内容不能为空"});return;}res.status(201).json(await momoRepo.createCommunityPost(memberId,member.nickname,{...req.body,title,body}));}));
momoRouter.post("/community/posts/:id/like",requireMember,asyncHandler(async(req,res)=>{const post=await momoRepo.toggleCommunityLike(String(res.locals.memberId),req.params.id);if(!post){res.status(404).json({error:"分享不存在"});return;}res.json(post);}));
momoRouter.get("/community/packages",requireMember,asyncHandler(async(req,res)=>{res.json(await momoRepo.communityPackages(String(req.query.kind||"")));}));
momoRouter.post("/community/packages",requireMember,asyncHandler(async(req,res)=>{const memberId=String(res.locals.memberId),member=await memberRepo.getById(memberId),kind=String(req.body?.kind||"");if(!member){res.status(404).json({error:"账号不存在"});return;}if(kind!=="skill"&&kind!=="app"){res.status(400).json({error:"共享类型无效"});return;}const file=req.body?.file||{},name=String(req.body?.name||"").trim(),ext=path.extname(String(file.name||"")).toLowerCase();if(!name||!String(file.url||"").trim()){res.status(400).json({error:"名称和文件不能为空"});return;}if(kind==="app"&&ext!==".exe"){res.status(400).json({error:"AI 应用分享仅支持 EXE"});return;}if(kind==="skill"&&!([".skill",".zip",".md",".txt",".json"].includes(ext))){res.status(400).json({error:"Skill 支持 .skill、.zip、.md、.txt 或 .json"});return;}res.status(201).json(await momoRepo.createCommunityPackage(memberId,member.nickname,{...req.body,kind,name,file:{name:String(file.name).slice(0,160),url:String(file.url),type:String(file.type||"application/octet-stream"),size:Number(file.size)||0}}));}));
